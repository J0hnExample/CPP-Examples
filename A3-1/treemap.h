...
treemap<K,T>::operator[](const K& key)
{
    ...
    if (foundNode)
    {
        T& nodeValue = treemap_node<K,T>::value_(*foundNode);
        // Return the value of the found node
        return nodeValue;
    }
    else
    {
        // Insert a new node with a default-constructed value
        T& newNode = insert_(key, T());
        // newNode is a pair of (node_ptr, bool)
